...
-- 8
SELECT film_id, title, description, length, rental_rate FROM film WHERE title IN ('Goodfellas');
...
-- 11
SELECT * FROM film ORDER BY rental_rate OFFSET 10 ROWS FETCH FIRST 10 ROW ONLY;
...
-- 15
SELECT 
	customer.customer_id, customer.first_name, customer.last_name, payment.amount, payment.payment_date, payment.staff_id
FROM
	customer
INNER JOIN 
	payment
